...
func (s *Service) reconcileChangeSetRevision(ctx context.Context, run *model.Run, rev *model.Revision) error {
	entry, err := s.changeSetStore.GetEntryBySlug(ctx, *run.ChangeSetId, rev.ComponentSlug)
...
func (s *Service) finalizeChangeSet(ctx context.Context, run *model.Run) error {
	...
	for i := range entries {
		...
		if err != nil {
			return fmt.Errorf("load component for orphan %q: %w", e.ComponentSlug, err)
		}
		if _, err := s.componentStore.Update(ctx, comp, map[string]any{
			"desired_state": model.ComponentDesiredStateOrphan,
		}); err != nil {
			return fmt.Errorf("orphan component %q: %w", e.ComponentSlug, err)
		}
	}
...
// checked here -- the slug-collision check in materializeChangeSetEntries
...
func (s *Service) checkChangeSetConflicts(ctx context.Context, cs *model.ChangeSet, entries []model.ChangeSetEntry) error {
	for i := range entries {
		...
		if err != nil {
			return status.Errorf(codes.Internal, "conflict check: query last deployed for %q: %v", e.ComponentSlug, err)
		}
		baseRevID, hasBase := cs.BaseHeadRevisions[e.ComponentSlug]
		...
		return status.Errorf(codes.FailedPrecondition,
			...
			e.ComponentSlug, baseRevID, currentRevID)
	}
	return nil
}
...
//     entry's slug for this env, then backfill the entry's component_id so
...
//     between attempts), then proceed without erroring on slug-already-exists.
...
//
// Returns the deployable components in entry order plus the set of slugs
// whose revision must be stamped change_type=DESTROY.
func (s *Service) materializeChangeSetEntries(
	...
	for i := range entries {
		...
		case model.ChangeSetEntryTypeCreate:
			if e.ModuleId == nil {
				return nil, nil, status.Errorf(codes.InvalidArgument, "CREATE entry %q is missing module_id", e.ComponentSlug)
			}
			...
			if e.ComponentId != nil {
				...
				if err != nil {
					return nil, nil, status.Errorf(codes.Internal, "failed to load materialized component for retry of CREATE %q: %v", e.ComponentSlug, err)
				}
				...
				if err != nil {
					return nil, nil, status.Errorf(codes.Internal, "failed to refresh component HEAD for retry of CREATE %q: %v", e.ComponentSlug, err)
				}
				comp = refreshed
			} else {
				if existing, err := s.componentStore.GetByApplicationEnvSlug(ctx, appID, envID, e.ComponentSlug); err == nil && existing != nil {
					return nil, nil, status.Errorf(codes.AlreadyExists,
						"change set creates slug %q but a component with that slug already exists in this environment", e.ComponentSlug)
				}
				mod, err := s.moduleStore.Get(ctx, *e.ModuleId)
				if err != nil {
					return nil, nil, status.Errorf(codes.InvalidArgument, "module %s for CREATE entry %q not found", *e.ModuleId, e.ComponentSlug)
				}
				...
				fresh := &model.Component{
					ApplicationId: appID,
					EnvironmentId: envID,
					Name:          e.ComponentSlug,
					Slug:          e.ComponentSlug,
					Kind:          kind,
				...
				if err != nil {
					return nil, nil, status.Errorf(codes.Internal, "failed to materialize component for CREATE entry %q: %v", e.ComponentSlug, err)
				}
				if err := s.changeSetStore.SetEntryComponentID(ctx, e.Id, saved.Id); err != nil {
					return nil, nil, status.Errorf(codes.Internal, "failed to backfill component_id on entry %q: %v", e.ComponentSlug, err)
				}
				comp = saved
			}
		...
		case model.ChangeSetEntryTypeUpdate, model.ChangeSetEntryTypeDestroy:
			if e.ComponentId == nil {
				return nil, nil, status.Errorf(codes.InvalidArgument, "%s entry %q is missing component_id", e.ChangeType, e.ComponentSlug)
			}
			comp, err := s.componentStore.Get(ctx, *e.ComponentId)
			if err != nil {
				return nil, nil, status.Errorf(codes.NotFound, "component for entry %q not found: %v", e.ComponentSlug, err)
			}
			if comp.ApplicationId != appID || comp.EnvironmentId != envID {
				return nil, nil, status.Errorf(codes.InvalidArgument,
					"component %s for entry %q does not belong to this run's (application, environment)", comp.Id, e.ComponentSlug)
			}
			if e.ChangeType == model.ChangeSetEntryTypeUpdate {
				...
				destroySlugs[comp.Slug] = true
			}
			deployable = append(deployable, *comp)

		default:
			return nil, nil, status.Errorf(codes.InvalidArgument, "unknown change_type %q on entry %q", e.ChangeType, e.ComponentSlug)
		}
	}
	return deployable, destroySlugs, nil
}
